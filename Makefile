...
RABBITMQ_IMAGE_NAME=$(DOCKER_REPOSITORY)/rabbitmq
RABBITMQ_REPLICAS=3
...
docker-rabbitmq:
	$(SUDO) docker pull $(RABBITMQ_IMAGE_NAME):$(RABBITMQ_IMAGE_TAG) || $(SUDO) docker build -t $(RABBITMQ_IMAGE_NAME):$(RABBITMQ_IMAGE_TAG) $(RABBITMQ_DOCKER_DIR) && $(SUDO) docker push $(RABBITMQ_IMAGE_NAME):$(RABBITMQ_IMAGE_TAG)
